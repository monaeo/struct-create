...
import (
	...
	// need pq for the NullTime object
	_ "github.com/lib/pq"
)
...
func writeStructs(schemas []ColumnSchema) (int, error) {
	...
	header += "//\n// Do not modify. This file was generated by struct-create. Modifications will be overwritten in the future.\n//\n\n"
...
func getSchema() []ColumnSchema {
	dsn := fmt.Sprintf("%s:%s@(%s:%s)/information_schema?parseTime=true&interpolateParams=true",
...
func formatName(name string) string {
	...
	for _, p := range parts {
		...
		if p == "id" || p == "ip" || p == "url" {
			newName += strings.ToUpper(p)
		} else {
			newName += strings.Replace(p, string(p[0]), strings.ToUpper(string(p[0])), 1)
		}
	}
	return newName
}
...
func goType(col *ColumnSchema) (string, string, error) {
	...
	case "date", "time", "datetime", "timestamp":
		if col.IsNullable == "YES" {
			gt, requiredImport = "pq.NullTime", "github.com/lib/pq"
		} else {
